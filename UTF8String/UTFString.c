...
void utf32_to_8(const uint32_t* char_array, size_t array_size, char* ret_array, size_t* ret_array_size)
{
    //if ret_array_size is zero or ret_array is NULL, then calculate the array size
    if (*ret_array_size == 0 || ret_array == NULL) {
        size_t ret_size = 0;
...
void utf8_to_16(const char* char_array, size_t array_size, uint16_t* ret_array, size_t* ret_array_size)
{
    //if ret_array_size is zero or ret_array is NULL, then calculate the array size
    if (*ret_array_size == 0 || ret_array == NULL) {
        size_t ret_size = 0;
...
void utf16_to_8(const uint16_t* char_array, size_t array_size, char* ret_array, size_t* ret_array_size)
{
    //if ret_array_size is zero or ret_array is NULL, then calculate the array size
    if (*ret_array_size == 0 || ret_array == NULL) {
        size_t ret_size = 0;
...
void utf_grow(UTFString* str, size_t needed_size) {
    ...
    if (str->raw_size < calculated_needed) {
        char* new_block = realloc(str->data, calculated_needed);
        if (new_block) {
            str->data = new_block;
...
{
    return (utf_sv_find_last(sv, with) == (int)(sv.count - with.count));
}
